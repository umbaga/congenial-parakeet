module.exports = function(app, pg, async, pool, itemtypes, common) {
    ...
    app.get('/api/adm/equipments', function(req, res) {
        ...
        pool.connect(function(err, client, done) {
            ...
            sql += '    \'name\', cat."itemName", \'id\', cat."id"';
            sql += ') AS "category"';
            sql += ', json_build_object(';
            ...
            sql += ', case when impweap."range" IS NULL then \'{}\' else json_build_object(';
            sql += '    \'damage\', json_build_object(';
            sql += '        \'dieCount\', dmgdice."dieCount"';
            sql += '        , \'dieType\', dmgdice."dieType"';
            sql += '        , \'rendered\', concat_ws(\'d\', dmgdice."dieCount"::text, dmgdice."dieType"::text)';
            sql += '    )';
            sql += '    , \'damageType\', json_build_object(';
            sql += '        \'id\', impweap."damageTypeId"';
            sql += '        , \'name\', dmgtype."itemName"';
            sql += '    )';
            ...
            sql += ', description.description';
            sql += ', dmgdice."dieCount", dmgdice."dieType"';
            sql += ', dmgtype."itemName"';
            sql += ', impweap."range", impweap."damageTypeId", impweap.range';
