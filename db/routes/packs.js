module.exports = function(app, pg, async, pool, itemtypes, modules) {
    ...
    app.get('/api/adm/equipment/packs', function(req, res) {
        ...
        pool.connect(function(err, client, done) {
            ...
            sql += ', case when count(asseq) = 0 then \'[]\' else json_agg(';
            sql += '    (';
            sql += '        SELECT x FROM (';
            sql += '            SELECT asseq.id, asseqtbl.cost, asseqtbl.weight, asseq."itemName" as "name"';
            sql += '            , pack."assignedCount"';
            sql += '            , case when asscntunit."itemCount" IS NULL then 1 else asscntunit."itemCount" end AS "count"';
            sql += '            , case when asscntunit."unitName" IS NULL then \'\' else asscntunit."unitName" end AS "unit"';
            sql += '        ) x';
            sql += '    )';
            ...
            sql += ', equip.cost, equip.weight';
            sql += ', cat."itemName", cat."id"';
            sql += ', rsrc."itemName", rsrc."id"';
